		...
		while(true) {
			...
			if(sign.equals("SearchOn")) { //검색 요구
				...
				*/
				...
				saveList.add(new String[]{필드, 필드, 필드, 필드 ...}) -> 정훈이가 리스트 사용한다고 함
				
				*/
				
				/* ---- 여긴 수정 필요 ----
				for(int i = 0; i < matchNum; i ++){ // matchNum : 보낼 경기 수
					if(int == matchNum-1)
					{
						try {
							searchMsg = "FAINALLMSG$" + 정보 1 + "&&" + 정보2 + "&&" + 정보3 + .... );
				...
				*/
				...
				try {
					out.writeUTF("SAVEPAGE$"); // 세이브 페이지로 이동
				}catch(IOException e) {}
				
			}else if(sign.equals("MYLODING")) {
				boolean result = false; // 검색 성공 조건 - 필요에 따라 형태 바꾸기
